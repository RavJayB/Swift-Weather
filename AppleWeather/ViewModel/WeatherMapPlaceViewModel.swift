...
//        https://api.openweathermap.org/data/3.0/onecall?lat=50&lon=50&units=metric&appid=19002a6736dce98b2d635896cd45fd2d
        guard let url = URL(string: "https://api.openweathermap.org/data/3.0/onecall?lat=\(lat)&lon=\(lon)&units=metric&appid={Add your openweather API key here}") else {
            print("Invalid url")
            return
        }
    ...
    func fetchWeatherCityData(for cityName: String) async throws {
            ...
            let cityWeatherURL = "https://api.openweathermap.org/data/2.5/weather?q=\(cityName)&units=metric&appid={Add your openweather API key here}"
